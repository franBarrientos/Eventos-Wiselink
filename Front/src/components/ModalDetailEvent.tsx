...
import { SubscribeToEvent } from "../api/services/EventsService.ts"
import { SubscribeAddDTO } from "../api/dtos/input.ts"
import { toast } from "react-toastify"

type Props = EventDTO

const ModalDetailEvent: FunctionComponent<Props> = (event) => {
  const { user, setUser } = useApp()

  function subscribe() {
    const data: SubscribeAddDTO = {
      Event: event.Id,
      User: user?.Id,
    }
    SubscribeToEvent(data)
      .then((r) => {
        const userUpdated: UserDTO = {
          ...user,
          Events: r.eventsSubscribed,
        }
        localStorage.setItem("user", JSON.stringify(userUpdated))
        setUser(userUpdated)

        toast.success(`Subscribed to ${event.Title}`)
        showDetailsModalSubject.setSubject(false)
      })
      .catch((e) => {
    ...
    <section className="text-gray-600 body-font">
      <div className="px-5 py-16 mx-auto">
        ...
        <div className="inline-flex w-full justify-center mt-5">
          {user?.Events?.some((e) => e.Id === event.Id) &&
          new Date(event.Date).getTime() < new Date().getTime() ? (
            <>
              <a className="flex items-center  px-3 mr-1 py-2 bg-green-600 text-white font-semibold rounded">
                <svg
                  fill="none"
                  stroke="currentColor"
                  stroke-linecap="round"
